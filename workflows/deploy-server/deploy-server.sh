...
# ---DOC-START---
# summary: Full-stack workflow: firewall -> SSH -> Fail2Ban -> Nginx -> Grafana -> Portainer -> File Browser, each independently toggleable from a single .env.
# description: |
#   Orchestrates a full server deployment by running up to seven scripts in
#   sequence from a single `.env` config file. Every step is independently
#   toggleable — set the matching `DEPLOY_*` variable to `n` to
...
# interactive: true
# idempotent: false
# dependencies: standalone/install/apt/cli/install-firewalld-cli.sh, standalone/deploy/deploy-ssh.sh, standalone/deploy/deploy-fail2ban.sh, standalone/deploy/deploy-nginx.sh, standalone/deploy/deploy-grafana.sh, standalone/deploy/deploy-portainer.sh, standalone/deploy/deploy-filebrowser.sh
# ---DOC-END---
...
STEP_TOGGLES=(
    DEPLOY_FIREWALL DEPLOY_SSH DEPLOY_FAIL2BAN
...
if [[ $TOTAL_STEPS -eq 0 ]]; then
    echo "[!] Every step is disabled (all DEPLOY_* toggles are 'n') — nothing to do."
    exit 1
fi
