...
# ---DOC-START---
# summary: Recursively set permissions to 644 on files in a path.
# description: |
...
echo " Action:      Set permissions (644)"
...
if [ "$DRY_RUN" = false ] && [ "$ASSUME_YES" = false ]; then
    read -rp "Are you sure you want to set permissions to 644? (y/N): " CONFIRM
    if [[ ! "$CONFIRM" =~ ^[Yy]$ ]]; then
...
if [ -f "$ABS_PATH" ]; then
    ...
    find "$ABS_PATH" -type f -exec chmod 644 {} +
fi
